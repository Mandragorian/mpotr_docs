...
Our work is heavily based on the mpotr \cite{mpotr} paper. We try to specify
the subprotocols mentioned in \cite{mpotr}. Namely we propose a Deniable
Signature Key Exchange (DSKE) based on the pairwise triple Diffie-Hellman
protocol CITE TRIPLE DH HERE, and as a  Group Key Agreement (GKA) we use the
protocol proposed in \cite{mpenc} but using classic Diffie-Hellman (ie not ECDH).
